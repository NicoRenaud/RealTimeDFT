...
from PyQuante.NumWrap import zeros,arcsinh,where
import numpy as np
import sys

##############################################################
## Function to simulate gatherv caus ei have issues with it
#############################################################

def gather_at_root(source,send_counts,disp,rank,comm):

    # master copy its onw data
    # and the receive the data of all others
    print send_counts
    sys.exit()
    if rank == 0:

        # initialize the dest
        size_dest = np.sum(send_counts)
        print size_dest
        dest = np.zeros(size_dest)

        # put the data of root in the final vector
        dest[:send_counts[0]] = source

        # receive data from slaves
        for iC in range(1,comm.size):

            recvbuffer = np.zeros(send_counts[iC])
            comm.Recv(recvbuffer,surce=iC)
            dest[disp[iC]:disp[iC]+send_counts[iC]] = recvbuffer
            if _debug_:
                    print '\t\t\t Proc [%03d] : data received from [%03d]' %(0,iC)

    # the slaves send their stuff
    else:
        dest = None
        comm.Send(source,dest=0)
...
########################################################
## TOP FUNCTION FOR THE XC FUNCTIONAL
########################################################


def XC_mpi(dens,gamma,**kwargs):
    ...
    # get the info we need about the functional
    ...
    # compute the exchange part of the functional
    if xfuncs[functional]:
        if derivative == 'analyt' and analyt[functional]:
            ...
            print 'MPI only available for analytic gradient'
            sys.exit()
            #fx,dfxdna,dfxdnb,dfxdgaa,dfxdgab,dfxdgbb = xfuncs[functional](dens,gamma)
    ...
    # compute the correlation part of the functional
...
########################################################
## Wrapper function to get all at once .... 
########################################################
