...
def main(argv):
	...
	if rank == 0:
		...
		print '== MPI version on %d procs                      ==' %(comm.size)
		print '==================================================\n'
	...
	# at the end only the master procs have all the Ins !
	if rank == 0:
		print '\n\t Compute the integrals and form the matrices'
	S,Hcore,Ints = Ints_MPI.getints_mpi(bfs,mol,rank,comm,_debug_=True)
	...
	# set up the molecular grid for the density integration
	if rank == 0:

		# only the master holds the entire grid
		# we could distribute the grid over all the
		# procs equally as well ...
	...
	# the other procs have a None for gr
	else:
		gr = None
	...
	# all the procs contribute to that function to
	# parrallelize the calculation of Exc and XC
	L,C,Cp,F0,F0p,D,Dp,P,X = dft_mpi(mol,bfs,S,Hcore,Ints,gr,args.xcf,rank,comm,MaxIter=args.MaxIter,eps_SCF=args.eps_SCF,_diis_=args.diis)
	...
	# Only the master procs performs the tests
	if _test_ and rank == 0:
	...
	#mater procs determine the different units/constants
	# and declare the containers it needs
	if rank == 0:

		# pick up the correct time unit
		...
		# declare the different vector we need
	...
	for iT in range(0,args.nT):
		...
		if _predict_ and iT < args.nT-1:
			...
			comm.Barrier()
			fp = compute_F_mpi(d,Hcore_p,X,gr,args.xcf,Ints,nelec,rank,comm,mup_p1)
		...
		# new Fock matrix
		comm.Barrier()
		Fp = compute_F_mpi(D,Hcore_p,X,gr,args.xcf,Ints,nelec,rank,comm,mup)

	if rank == 0:
		t1 = time.time()
		sys.stdout.write('\r\t Simulation done at 100 percent')
		sys.stdout.write('\t Dynamics simulated in %f sec.' %(t1-t0))
	...
	##########################################################
	if _export_mo_dynamics_ and rank == 0 :
